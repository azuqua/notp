...
class GenServer extends EventEmitter {
   ...
   * @memberof NetKernel
   *
   * @param {NetKernel.NetKernel} kernel
   *
   */
  constructor(kernel) {
    super();
  ...
  start(name) {
    ...
    this.once("stop", _.partial(this._kernel.removeListener, this._id, handler).bind(this._kernel));
    return this;
  }
   ...
   * @instance
   *
   * @param {NetKernel} [kernel] - NetKernel to set on this instance.
   *
   * @return {NetKernel} NetKernel of this instance.
   *
   */
  kernel(kernel) {
    if (kernel) {
   ...
   *
   * @return {GenServer} This instance.
   *
   */
  reply(from, data) {
  ...
  _parse(data, stream, from) {
    ...
    if (!stream.error) {
      ...
      this.emit(job.event, job.data, from);
    }
