...
class GenServer extends EventEmitter {
  ...
  multicall(nodes, id, event, data, cb, timeout=Infinity) {
    return this._kernel.multicall(nodes, id, {
      event: event,
      data
    }, cb, timeout);
  }
  ...
  _parse(data, stream, from) {
    ...
    if (stream.done === true && data && !this._streams.has(stream.stream)) {
      data = this.decodeSingleton(data);
      this.emit(data.event, data.data, from);
      return this;
    } else if (stream.done === true && stream.error !== undefined && !this._streams.has(stream.stream)) {
      return this;
    }
