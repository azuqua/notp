...
class NetKernel extends EventEmitter {
  ...
  connect(node, cb) {
    ...
    const conn = new Connection(this._ipc, node);
    conn.start();
  ...
  call(node, event, data, cb, timeout=Infinity) {
    const id = crypto.randomBytes(RAND_BYTE_LEN).toString("hex");
  ...
  _streamLocal(event, tag, data) {
    ...
    const streamID = crypto.randomBytes(RAND_BYTE_LEN).toString("hex");
