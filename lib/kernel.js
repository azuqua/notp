...
class NetKernel extends EventEmitter {
  ...
  call(node, event, data, cb, timeout=Infinity) {
    var id = crypto.randomBytes(RAND_BYTE_LEN).toString("hex");
    if (node.id() === this._id) {
      this._streamLocal(event, id, data);
    }
    else if (!this._sinks.has(node.id())) return null;
    else {
      var err = this._streamData(node, event, id, data);
  ...
  callSingleton(node, event, data, cb, timeout=Infinity) {
    ...
    this._setupSingletonReply(node, id, timeout, cb);
  ...
  multicallSingleton(nodes, event, data, cb, timeout=Infinity) {
    if (typeof cb !== "function") {
      return nodes.map(_.partialRight(this.callSingleton, event, data, _, timeout).bind(this));
    }
  ...
  _streamLocal(event, tag, data) {
    var streamID = crypto.randomBytes(RAND_BYTE_LEN).toString("hex");
    ...
    var dataListener = _.partial(this._sendLocal, event, tag, _.clone(rstream)).bind(this);
  ...
  _streamData(node, event, tag, data) {
    ...
    var dataListener = _.partial(this._sendData, data, conn, event, tag, _.clone(rstream)).bind(this);
    data.on("data", dataListener);
  ...
  _rstream(node, id, timeout=Infinity) {
    var pstream = new stream.PassThrough({objectMode: true});
  ...
  static _coerceStream(data) {
    if (data instanceof stream.Stream) {
      ...
      var nstream = new stream.PassThrough();
      async.nextTick(() => {
        nstream.write(Buffer.from(JSON.stringify(data)));
        nstream.end();
      });
      return nstream;
    }
  ...
  static _decodeBuffer(data) {
    ...
    else if (!util.isObject(data)) return data;
