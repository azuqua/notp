...
class NetKernel extends EventEmitter {
   ...
   *
   * @return {NetKernel} This instance.
   *
   */
  start(opts) {
   ...
   * @instance
   *
   * @param {Node} [node] - Node to set as "self" on this netkernel.
   *
   * @return {Node} Self-referencing node of this instance.
   *
   */
  self(node) {
   ...
   * @instance
   *
   * @param {Node} node - Node to create socket connection with.
   ...
   * @instance
   *
   * @param {Node} node - Node to check existing connection to.
   * 
   ...
   * @instance
   *
   * @param {Node} node - Node to grab connection of.
   *
   * @return {Connection} Connection of `node`, or null if it doesn't exist.
   *
   */
  connection(node) {
   ...
   * @instance
   *
   * @param {Node} node - Node to send `data` to.
   ...
   * @param {Node} from.node - Node representing the sender.
   * @param {Stream|Buffer|String} data - Data to send with this message. `data` is coerced into a stream format, throwing on failed coersion.
   *
   * @return {NetKernel} This instance.
   *
  ...
  _streamData(node, event, tag, data) {
    ...
    var dataListener = _.partial(this._sendData, conn, event, tag, rstream).bind(this);
    data.on("data", dataListener);
    data.once("end", _.partial(this._finishData, conn, event, tag, rstream).bind(this));
    data.once("error", (err) => {
      data.removeListener("data", dataListener);
      if (data.resume) data.resume();
      this._sendError(conn, event, tag, rstream, err);
    });
   ...
   *
   */
  _sendData(conn, event, tag, stream, data) {
   ...
   *
   */
  _finishData(conn, event, tag, stream) {
    if (stream.done) return this;
    stream.done = true;
    return this._sendData(conn, event, tag, stream, null);
  }
   ...
   *
   */
  _sendError(conn, event, tag, stream, data) {
    if (stream.done) return this;
    stream.done = true;
    stream.error = NetKernel._encodeError(data);
    data = null;
    return this._sendData(conn, event, tag, stream, data);
  }
