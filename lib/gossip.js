...
class GossipRing extends GenServer {
  ...
  /**
   *
   * Registers table `table` as a 'child' of this gossip ring. When this instance
   * is stopped or departs from it's ring, these tables are purged of data and
   * repartitioned across the cluster it used to belong to.
   *
   * @method registerTable
   * @memberof NetKernel.GossipRing
   * @instance
   *
   * @param {NetKernel.StateTable} table - Table to register with this gossip ring.
   *
   * @return {GossipRing} This instance.
   *
   */
  ...
  join(ringID) {
    ...
    this.once("leave", _.partial(this._kernel.removeListener, this._ringID, handler).bind(this._kernel));
    return this;
  }
  ...
  meet(node) {
    ...
    this._kernel.cast(node, this._ringID, GossipRing._encodeState({
      type: "join",
      event: "ring",
      actor: this._actor,
      data: msg,
  ...
  route(nodes, event, msg, actor, clock, n = 1) {
    ...
    this._kernel.abcast(nodes, this._ringID, GossipRing._encodeState({
      type: "update",
      event: event,
      actor: actor,
      data: msg,
  ...
  _closeTables(sendRing) {
    ...
    var count = this._tables.size;
    this._tables.forEach((val, key) => {
      val.once("purge", () => {
        if (--count === 0) {
          this._ringID = null;
          this._makeDisconnects(sendRing.nodes());
